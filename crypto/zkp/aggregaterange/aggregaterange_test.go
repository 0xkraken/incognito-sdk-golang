...
func TestInnerProductProveVerify(t *testing.T) {
	for k := 0; k < 1; k++ {
		...
		for i := range wit.a {
			wit.a[i] = new(crypto.Scalar).FromUint64(uint64(rand.Intn(1000000)))
			wit.b[i] = new(crypto.Scalar).FromUint64(uint64(rand.Intn(1000000)))
		}
...
func TestInnerProductProveVerifyUltraFast(t *testing.T) {
	...
	for k := 0; k < count; k++ {
		...
		if k == 0 {
			wit.p = new(crypto.Point).ScalarMult(aggParam.u, c.Add(c, new(crypto.Scalar).FromUint64(1)))
		} else {
			wit.p = new(crypto.Point).ScalarMult(aggParam.u, c)
		}
		...
		for i := range wit.a {
			wit.p.Add(wit.p, new(crypto.Point).ScalarMult(aggParam.g[i], wit.a[i]))
			if k == count -1 {
				wit.p.Add(wit.p, new(crypto.Point).ScalarMult(aggParam.h[i], wit.a[i]))
			} else {
				wit.p.Add(wit.p, new(crypto.Point).ScalarMult(aggParam.h[i], wit.b[i]))
			}
		}
...
func obfuscatePoint(value *crypto.Point) *crypto.Point {
	for {
		k := value.GetKey()
		r := common.RandInt() % len(k)
		i := common.RandInt() % 8
		k[r] ^= (1 << uint8(i))
		after, err := new(crypto.Point).SetKey(&k)
		if err == nil {
			return after
		}
	}
}
func obfuscateScalar(value *crypto.Scalar) *crypto.Scalar {
	for {
		k := value.GetKey()
		r := common.RandInt() % len(k)
		i := common.RandInt() % 8
		k[r] ^= (1 << uint8(i))
		after, err := new(crypto.Scalar).SetKey(&k)
		if err == nil {
			return after
		}
	}
